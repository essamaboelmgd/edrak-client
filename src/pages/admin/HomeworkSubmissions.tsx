...
export default function AdminHomeworkSubmissions() {
    ...
    const handleGradeSubmit = async () => {
        ...
        // Max score validation - Using conditional check since admin view might not have full homework details in submission object initially
        // We will fallback to allow if totalPoints is missing to avoid blocking admin, or ideally it should be present.
        // Assuming ISubmission interface is updated or response includes it. 
        // Let's rely on what's available. The backend response for submission includes 'homework' populated with 'totalPoints'.
        
        const maxScore = (selectedSubmission as any).homework?.totalPoints;
         const enteredScore = gradeData.score ? Number(gradeData.score) : 0;

        if (maxScore !== undefined && maxScore > 0 && enteredScore > maxScore) {
             toast({
                title: 'خطأ',
                ...
                <Flex justify="space-between" align="center">
                    <VStack align="start" spacing={2}>
                        <HStack>
                            ...
                            />
                            <Heading size="lg">إدارة تسليمات الواجب (إدارة)</Heading>
                        </HStack>
                        <Text opacity={0.8}>عرض وتصحيح واجبات الطلاب - لوحة الإدارة</Text>
                    </VStack>
                </Flex>
            </Box>
