...
export default function EditCourseModal({ course, callback, trigger }: EditCourseModalProps) {
    ...
    useEffect(() => {
        if (isOpen && course) {
            ...
            const eduLevelId = course?.educationalLevel?._id || course?.educationalLevel || "";
            reset({
                title: course?.title || "",
                description: course?.description || "",
                price: course?.price || 0,
                discount: course?.discount || 0,
                educationalLevel: eduLevelId,
                courseSection: course?.courseSection?._id || course?.courseSection || "",
                startDate: course?.startDate ? new Date(course.startDate).toISOString().split('T')[0] : "",
                endDate: course?.endDate ? new Date(course.endDate).toISOString().split('T')[0] : "",
            });
            if (course.poster) {
                setImagePreview(getImageUrl(course.poster));
            }
            setSelectedImage(null);
            // Fetch sections for the course's educational level
            if (eduLevelId) {
                fetchCourseSections(eduLevelId);
            }
        }
    }, [isOpen, course, reset]);

    useEffect(() => {
        if (isOpen && watchedEducationalLevel) {
    ...
    const onSubmit = async (values: CourseFormData) => {
        try {
            ...
            // Handle courseSection - send empty string to remove, or the ID to assign
            formData.append("courseSection", values.courseSection || "");
    ...
    return (
        <>
            ...
            <Modal isOpen={isOpen} onClose={onClose} size="xl">
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>تعديل الكورس</ModalHeader>
