...
import { coursesService, ICourseAdmin } from "../services/coursesService";
...
export default function EditCourseModal({ course, isOpen, onClose, onSuccess }: EditCourseModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            fetchTeachers();
            if (course) {
                const eduLevelId = course?.educationalLevel?._id || "";
                const teacherId = course?.teacher?._id || "";
                const sectionId = course?.courseSection?._id || "";
                reset({
                    title: course?.title || "",
                ...
                if (course.poster || course.thumbnail) {
                    setImagePreview(getImageUrl(course.poster || course.thumbnail!));
                }
            }
        }
    }, [isOpen, course, reset]);
    ...
    const onSubmit = async (values: CourseFormData) => {
        ...
        try {
            ...
            const response = await coursesService.updateCourse(course._id, formData);
            toast({
                status: "success",
                description: response.message || "تم تحديث الكورس بنجاح",
            });
            onClose();
            reset();
