...
interface LessonQuestionBankTabProps {
    lessonId: string;
}

export default function LessonQuestionBankTab({ lessonId }: LessonQuestionBankTabProps) {
    ...
    const handleSave = async (data: ICreateQuestionBankRequest | any) => {
        try {
            if (selectedQuestion) {
                ...
                await questionBankService.updateQuestion(selectedQuestion._id, data);
                toast({
                    status: 'success',
